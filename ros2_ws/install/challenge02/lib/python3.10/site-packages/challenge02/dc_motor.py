...
Motor_sys: Simulador de motor (PLanta del controlador PID)
"""
...
class DCMotor(Node):
    def __init__(self):
        ...
        # Creanmos un SERVIDOR y ledecimos alaccion que va arealziar cuando se llame
        self.srv = self.create_service(SetProcessBool, 'EnableMotors', self.service_callback)
    ...
    def timer_cb(self):   
        ...
        #self.get_logger().info(f'DC_Motor data {self.motor_output_msg}')   

    #Subscriber Callback
    def input_callback(self, input_sgn):
        self.input_u = input_sgn.data

    # Service callback (Llenamos la estrucutra del custom interface)
    def service_callback(self, request, response):
        # Activacion (Request)
        if request.enable:
            self.simulation_running = True
            self.get_logger().info('DC Motor Simulation Started!!')
            response.success = True
            response.message = 'DC Motor Simulation Started'
        # Desactivacion
        else:
            # (Response)
            self.simulation_running = False
            self.get_logger().info('DC Motor Simulation Stopped!!')
            response.success = True
            response.message = 'DC Motor Simulation Stopped'
        
        return response

    # Lecura de parametros en vivo
